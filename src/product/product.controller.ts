import {Body, Controller, Delete, Get, Param, Post, UploadedFiles, Put, UseInterceptors} from "@nestjs/common";
...
export class ProductController {
    ...
    @Put('update/:id')
    @UseInterceptors(FileFieldsInterceptor([{name: 'picture', maxCount: 1}]))
    update(@UploadedFiles() files, @Param('id') id: string, @Body() product: Product): Observable<UpdateResult>{
        const {picture} = files
